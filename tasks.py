...
PARTIAL_OPTIMIZER_TYPE=Callable[[Iterator[nn.Parameter]], optim.Optmizer]

class TrainingTask(pl.LightningModule):
    def __init__(self, optimizer: PARTIAL_OPTIMIZER_TYPE) -> None:
        super().__init__()
        self.optimizer : optimizer = PARTIAL_OPTIMIZER_TYPE
...
class MNISTClassification(TrainingTask):
    def __init__(self, model : nn.Module, optimizer : PARTIAL_OPTIMIZER_TYPE, loss_function : nn.Module) -> None:
        ...
        nrof_classes = 10
        self.train_accracy = Accuracy(task="multiclass", num_classes=nrof_classes)
    ...
    def training_step(self, batch, _) -> Tensor:
        ...
        self.train_accracy(logits, labels)
        self.log("train_loss", loss, on_step=True, on_epoch=True)
        self.log("train_accuracy", self.train_accracy, on_step=False, on_epoch=True)
        return loss
